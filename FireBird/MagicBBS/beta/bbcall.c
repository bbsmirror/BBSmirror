...
{
  int sockfd;
  char PAGER_NO[7], PASSWD[7], TRAN_MSG[17];
  char Msg[64], trn[512], result[4096], sendform[512];
  struct sockaddr_in serv_addr; 
  struct hostent *hp;
  time_t now;
  struct tm *pt;
  ...
  if ((!vget(0, 0, "請輸入您要傳呼的號碼：0941-", PAGER_NO, sizeof(PAGER_NO), DOECHO) ||
         !vget(2, 0, "請輸入傳呼訊息：", TRAN_MSG, sizeof(TRAN_MSG), DOECHO)))
  {
    vmsg("放棄傳呼");
    return;
  }
  ...
  move(3, 0);
  prints("你所要傳呼的號碼：0941-%s", PAGER_NO);
  move(4, 0);
  prints("你所要傳呼的訊息：%s", TRAN_MSG);
  ...
  sprintf(result, "User %s called 0941%s \nMsg = %s\n",
            cuser.userid, PAGER_NO, TRAN_MSG);
  f_cat(LOG_0941, result);
	
  sprintf(trn,"PAGER_NO=%s&PASSWD&TRAN_MSG=%s&MSG_TYPE=NUMERIC&NOW=on&year=%d&month=%d&day=%d&hour=%d&min=%d",
               PAGER_NO,TRAN_MSG,
               1900+pt->tm_year,pt->tm_mon,pt->tm_mday,pt->tm_hour,pt->tm_min);	
  sprintf(sendform, "POST %s HTTP/1.0\nReferer: %s\n%sContent-length:%d\n\n%s",
