...
/*check if five*/
       ...
       //paul    if(tlivef>=2 && winner==0)    bandhand(4);
       //paul    if(livethree>=2 && tlivef ==0) bandhand(3);
...
int user_guide()
{
        ...
        outs("¤W:¡ô¤U:¡õ     ¥ª:¡ö¥k:¡÷");
        move(9,40);
        outs("Space:¤U¤l     q:Â÷¶}(ctrl+x)");
        move(10,40);
     //paul   outs("t:°e°T®§");
        move(11,40);
//paul        outs("¶Â¥ý¦³¸T¤â    ­n¦hª`·N!");
        return;
             ...
             int x4,int y4,int x5,int y5,int x6,int y6)
{
        ...
        if(n_black==4)
         {
    //paul       if(playboard[x1][y1]== black && playboard[x6][y6]== black)
    //paul        bandhand(6);
...
//by paul
              ...
              struct screenline* screen0 = calloc(t_lines, sizeof(struct screenline));
              memcpy(screen0, big_picture, t_lines * sizeof(struct screenline));    
                add_io(0, 0);
             //   my_write(opponent->pid,"¿é¤J°T®§:");
                add_io( fd, 0);
              memcpy(big_picture, screen0, t_lines * sizeof(struct screenline));
              free(screen0);
              redoscr();          
                clrtoeol();
                continue;
